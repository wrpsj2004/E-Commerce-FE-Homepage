...
function Index() {
  ...
  function showImage(item) {
    if (item.img !== undefined) {
      ...
      return (
        <img className="card-img-top" height="190px" src={imgPath} alt="" />
      );
    }
  ...
  return (
    <>
          ...
          {products.length > 0 ? (
            products.map((item) => (
              <div className="col-3 mt-3" key={item.id}>
                <div className="card card-img-top img-fluid">
                  {showImage(item)}
      ...
      <MyModal id="modalCart" title="ตะกร้าของฉัน">
        ...
        <div className="text-center">
          จำนวน {sumQty} รายการ รวมทั้งหมด {sumPrice.toLocaleString("th-TH")}{" "}
          บาท
        </div>
        <div className="mt-3">
          <div className="alert alert-info">
            <div>โปรดโอนเงินไปยังบัญชี</div>
            <div>กรุงไทย นายวรภพ ศรีแจ่ม 111-222-33333</div>
          </div>
          <div>
            <div>ชื่อผู้ซื้อ</div>
            <input
